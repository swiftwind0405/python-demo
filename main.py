...
def main(input_path: Path, output_path: Path) -> None:
    ...
    if missing:
        raise ValueError(f"缺少目标日期列: {missing}")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--input",
        default="10月考勤.xlsx",
        help="输入考勤表路径，默认为当前目录下的 10月考勤.xlsx",
    )
    parser.add_argument(
        "-o",
        "--output",
        help="输出文件路径，默认在输入文件同目录生成 “原名-加班统计.xlsx”",
    )
...
if __name__ == "__main__":
    ...
    if cli_args.output:
        ...
        output_path = input_path.with_name(f"{input_path.stem}-加班统计.xlsx")
